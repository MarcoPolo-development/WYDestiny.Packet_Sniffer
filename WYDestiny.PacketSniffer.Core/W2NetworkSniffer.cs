...
namespace WYDestiny.PacketSniffer.Core
{
    ...
    /// <summary>
    /// Used by <c>WYDestinyNetworkSniffer</c> to raise events of
    /// received packets.
    /// </summary>
    public class WYDestinyPacketEventArgs : EventArgs
    {
        ...
        public WYDestinyPacketEventArgs(PacketHeader packetHeader, PacketDirection direction, IPHeader ipHeader, 
    ...
    /// that filters and decrypts all network packets used by WYDestiny protocol.
    /// </summary>
    public class WYDestinyNetworkSniffer : NetworkSniffer
    {
        /// <summary>
        /// Event that will be invoked when a packet is received.
        /// </summary>
        public event EventHandler<WYDestinyPacketEventArgs> PacketReceived;
        ...
        public WYDestinyNetworkSniffer(IPAddress interfaceAddress, IPAddress address, 
            int port)
        ...
        public WYDestinyNetworkSniffer(IPAddress interfaceAddress, int port)
        ...
        private void AddFragment(IPAddress address, int id, int offset, int length)
        {
            ...
            WYDestinyMarshalHelper.BufferCopy(m_buffer, offset, m_fragmentedBuffers[address][id], 0, length);
        ...
        private byte[] ConsolidateFragments(IPAddress address)
        {
            ...
            {
                WYDestinyMarshalHelper.BufferCopy(kvp.Value, 0, buffer, currentOffset, kvp.Value.Length);
            ...
            else // Data >= sizeof(PacketHeader)
            {
                    ...
                    { // Add the last fragment to the list
                        ...
                        {   // We need a do-while loop, because we may get two messages
                            ...
                            do
                            {
                                ...
                                WYDestinyPacketSecurity.Decrypt(pMsgBuffer, pSize, currentOffset);
                                ...
                                WYDestinyMarshalHelper.BufferCopy(msgBuffer, currentOffset, packetBuffer, 0, pSize);

                                PacketReceived.Raise(this, new WYDestinyPacketEventArgs(pHeader, direction, ipHeader, 
                ...
                else // No Fragmentation...
                {
                    ...
                    do
                    {
                        ...
                        WYDestinyPacketSecurity.Decrypt(data, pSize, currentOffset);
                        ...
                        WYDestinyMarshalHelper.BufferCopy(m_buffer, currentOffset, packetBuffer, 0, pSize);

                        PacketReceived.Raise(this, new WYDestinyPacketEventArgs(pHeader, direction, ipHeader, tcpHeader, 
                            packetBuffer));
