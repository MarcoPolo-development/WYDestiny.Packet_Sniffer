...
namespace WYDestiny.PacketSniffer.Core  //TODO: Sometimes you still got to disable the Firewall as you dont receive incoming Packets
{
    ...
    public abstract class NetworkSniffer : IDisposable
    {
        ...
        private void SetFirewallRule(bool enabled)
        {
            ...
            if (!enabled)
                fwPolicy2.Rules.Remove("WYDestiny Packet Sniffer");
            else
            {
                // Creating a new rule.                     //TODO: Sometimes you still got to disable the Firewall as you dont receive incoming Packets
                INetFwRule2 rule = (INetFwRule2)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FWRule"));

                rule.Name = "WYDestiny Packet Sniffer";
                rule.Description = "WYDestiny Packet Sniffer - Allow network traffic";
